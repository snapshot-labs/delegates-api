...
export default function createWriters(indexerName: string) {
  const handleDelegateChanged: evm.Writer = async ({ event, source }) => {
    ...
    const previousDelegate: Delegate = await getDelegate(indexerName, fromDelegate, governanceId);
    previousDelegate.tokenHoldersRepresentedAmount -= 1;
    await previousDelegate.save();

    const newDelegate: Delegate = await getDelegate(indexerName, toDelegate, governanceId);
  ...
  const handleDelegateVotesChanged: evm.Writer = async ({ event, source }) => {
    ...
    const governanceId = source?.contract || '';
    const governance: Governance = await getGovernance(indexerName, governanceId);
    const delegate: Delegate = await getDelegate(indexerName, event.args.delegate, governanceId);
